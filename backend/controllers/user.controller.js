...
export const followUnfollowUser = async(req , res)=>{
  ...
  try{
    ...
    if(id === req.user._id.toString()){
      return res.status(400).json({message : "You can't follow yourself/Unfollow yourself"})
    }
    ...
    const userId = req.user._id;
     try{
        ...
        if(( !newPassword && currentPassword) || ( !currentPassword && newPassword)){
          res.status(400).json({message : "Please Provide the Both Current Password and New Password"})
        }
